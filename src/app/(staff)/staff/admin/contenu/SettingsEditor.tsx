...
export function SettingsEditor({ initialContent }: { initialContent: SettingsContent }) {
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <EditorSection title="Marche inter-membres">
        <label className="flex items-center gap-3 cursor-pointer group">
          <input
            type="checkbox"
            checked={content.marketEnabled}
            onChange={(e) => setContent((c) => ({ ...c, marketEnabled: e.target.checked }))}
            className="accent-gold w-4 h-4"
          />
          <div>
            <span className="text-text-primary text-sm font-medium group-hover:text-white transition-colors">
            ...
            <p className="text-text-muted text-xs mt-0.5">
              Les membres peuvent publier des annonces de vente, achat ou echange entre eux.
            </p>
          </div>
        </label>
      </EditorSection>
